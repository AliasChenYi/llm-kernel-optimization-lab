...
def benchmark(
    ...
    results = []
    candidates: list[tuple[str, int | None, object]] = [("torch", None, lambda: rms_norm_ref(x, weight))]
    for variant in variants:
        for num_warps in num_warps_values:
            label = f"triton:{variant}"
            if num_warps is not None:
                label = f"{label}:w{num_warps}"
            candidates.append((label, num_warps, lambda variant=variant, num_warps=num_warps: rms_norm_triton(
                x,
                weight,
                variant=variant,
                num_warps=num_warps,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["default"],
        choices=["default", "cache_weight", "two_pass", "two_pass_cache_weight"],
    )
